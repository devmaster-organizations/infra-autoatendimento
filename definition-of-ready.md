...
**User Story:**
Como dev, quero ter o ambiente de desenvolvimento padronizado para que todos os membros trabalhem com as mesmas versões e configurações.
...
**User Story:**
Como dev, quero TypeScript configurado em backend e frontend para que o código tenha tipagem estática e menos erros em tempo de execução.
...
**User Story:**
Como dev, quero subir todo o ambiente com um único comando para que qualquer membro ou avaliador execute o projeto facilmente.
...
**User Story:**
Como administrador, quero listar e consultar nós de navegação, usuários e logs pela API para ter visibilidade sobre o conteúdo do sistema.

**Critérios de Aceitação – DoR:**
- Contratos dos endpoints GET de nós, usuários e logs documentados (parâmetros, filtros, retorno).
- Proteção por role de Administrador definida para todos esses endpoints.
- Paginação ou limite de resultados definido para listagens longas.
...
**User Story:**
Como avaliador, quero o diagrama de casos de uso entregue na Sprint 1 para que os atores e fluxos do sistema estejam visualmente documentados.
...
**User Story:**
Como dev, quero testes unitários dos fluxos de navegação, consulta ao repositório, login e validação de token para garantir que o backend funciona conforme o esperado.

**Critérios de Aceitação – DoR:**
- Jest configurado e rodando no container de backend.
- Escopo de cada teste definido: qual service/controller será testado e quais cenários (sucesso e falha).
...
**User Story:**
Como administrador, quero inserir, editar, ativar/desativar e remover nós de navegação via API para gerenciar o conteúdo do chatbot sem alterar código.
...
**Critérios de Aceitação – DoR:**
- Contrato dos endpoints POST, PUT e DELETE de usuários documentado.
- Regra definida: apenas Administrador pode criar/remover usuários.
- Validação de campos (e-mail único, senha mínima) documentada.
...
**Critérios de Aceitação – DoR:**
- Estrutura do documento no banco definida (título, fonte, página, seção, texto do chunk).
- Contrato dos endpoints POST e PUT documentado.
- Formato de upload ou inserção de chunks definido e aprovado.
...
**User Story:**
Como dev, quero testes unitários dos fluxos de envio de pergunta, satisfação, logs e gestão de perguntas para garantir que as rotas push funcionam corretamente.

**Critérios de Aceitação – DoR:**
- Escopo de cada teste definido: cenários de sucesso e falha para RF05, RF07, RF08 e RF06 (PATCH).
...
**User Story:**
Como administrador, quero gerenciar nós de navegação, usuários e visualizar logs por uma interface visual para que alterações não exijam acesso direto ao banco.
...
**User Story:**
Como Secretária Acadêmica, quero ver as perguntas recebidas e atualizar seus status pela interface para que o atendimento seja organizado.
